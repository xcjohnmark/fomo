"""Setup classification and objective reason/risk extraction (Sections 40, 41, 67)."""
...
class SetupClassifier:
    ...
    @classmethod
    def classify(cls, token: NormalizedTokenData, score_result: MomentumScoreResult) -> SetupClassification:
        """Classify candidate into one of the 5 Section 41 setup profiles."""
        p5m = token.change_5m
        p1h = token.change_1h
        p4h = token.change_4h
        p24h = token.change_24h
        ...
        if -8.0 <= p5m < 0.0 and p1h >= 10.0 and token.buys > token.sells:
        ...
        if p5m >= 6.0 and p1h >= 12.0 and token.volume_5m / max(token.market_cap, 1.0) >= 0.04:
    ...
    def extract_reasons_and_risks(
        cls, token: NormalizedTokenData, score_result: MomentumScoreResult
        ...
        # Objective Reasons
        if token.change_5m > 0 and token.change_1h > 0:
            reasons.append(
                f"Positive price alignment across short timeframes (5M: +{token.change_5m:.1f}%, 1H: +{token.change_1h:.1f}%)"
            )
        if token.buyer_ratio >= 55.0:
            reasons.append(
                f"Broad participant breadth with {token.buyers} buyers vs {token.sellers} sellers ({token.buyer_ratio:.1f}% buyer ratio)"
            )
        if token.buy_tx_ratio >= 60.0:
            reasons.append(
                f"Strong buy transaction dominance ({token.buys} buys vs {token.sells} sells, {token.buy_tx_ratio:.1f}%)"
            )
        if token.liquidity_ratio >= 15.0:
            reasons.append(
                f"Healthy liquidity backing (${token.liquidity:,.0f}, {token.liquidity_ratio:.1f}% of MC)"
            )
        if len(reasons) < 2:
            reasons.append(f"Volume activity sustained at ${token.volume_5m:,.0f} (5M) and ${token.volume_1h:,.0f} (1H)")

        # Objective Risks (Rule 8: avoid confirmation bias; never hide risk)
        if token.top10_percentage >= 35.0:
            risks.append(
                f"Concentration risk: top 10 wallets control {token.top10_percentage:.1f}% of supply"
            )
        if token.liquidity < 15000.0:
            risks.append(
                f"Execution slippage risk: absolute pool liquidity is thin at ${token.liquidity:,.0f}"
            )
        if token.change_5m >= 15.0:
            risks.append(
                f"Overextended candle: rapid +{token.change_5m:.1f}% jump in 5m may invite immediate profit-taking"
            )
        if token.sellers > token.buyers:
            risks.append(
    ...
    def get_confirmation_and_invalidation(
        cls, token: NormalizedTokenData, classification: SetupClassification
        ...
        if classification == SetupClassification.PULLBACK_CONTINUATION:
            return (
                ...
                f"Breakdown below current support level (${token.price * 0.92:.6f})",
        ...
        invalidation = (
            f"Sharp 5M volume collapse or drop below support (${token.price * 0.90:.6f})"
        )
